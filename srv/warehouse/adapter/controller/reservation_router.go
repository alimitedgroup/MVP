...
func (r *ReservationRouter) Setup(ctx context.Context) error {
	...
	err := r.broker.RegisterRequest(ctx, broker.Subject(fmt.Sprintf("warehouse.%s.reservation.create", r.config.ID)), broker.NoQueue, r.reservationController.CreateReservationHandler)
	if err != nil {
		return err
	}
